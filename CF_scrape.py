...
for year in range (1950,2019,1):
    ...
    date = table[0].find_all('td', {'data-stat':'date_game'})
    time = table[0].find_all('td', {'data-stat':'time_game'})
    day =  table[0].find_all('td', {'data-stat':'day_name'})
    ...
    date_list = bs_to_list(date)
    time_list = bs_to_list(time)
    day_list = bs_to_list(day)
...
#%%
df = pd.DataFrame(zip(row_list, week_number_list, date_list, time_list,
                      day_list, winner_list, winner_pts_list, game_loc_list,
                      loser_list, loser_pts_list, notes_list), columns = 
                    ['row','week','date','time','day','winner','winner_pts',
                    'game_loc','loser','loser_pts','notes'])
df['year'] = year
